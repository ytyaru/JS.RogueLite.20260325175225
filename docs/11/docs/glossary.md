...
`斬る`には減少量という状態がある。この状態が今いくつかを接尾辞によって知らせる仕組みで実装している。サイズ制限により4字以内にすべきだ。すなわち`斬る`,`斬る+1`,`斬る+2`の3つの表記がありうる。

実装ロジック上は複数の特性があると`+1+2`のように末尾に特性の分だけ追記される。だがそれはどの特性がどの状態か読み取れない。よって複数の特性が接尾辞を返すような札を作ることを禁止する。これは設計制約とする。その禁止を強制する実装はしない。
